...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_category'])) {
        $cat = trim($_POST['category_name']);
        $pdo->prepare("DELETE FROM categories WHERE name = ?")->execute([$cat]);
    }
    ...
    if (isset($_POST['add_product'])) {
        ...
        $stmt->execute([$id, $_POST['name'], $_POST['category'], (float)$_POST['price'], $_POST['desc'], $imagePath]);
    }
          ...
          <div class="row">
            <div class="field grow"><label>Category</label>
              <select name="category">
                <?php
                $cats = $pdo->query("SELECT name FROM categories ORDER BY name ASC");
                while ($c = $cats->fetch()) {
            ...
            <div class="field grow"><label>Price (₱)</label>
              <input name="price" type="number" min="0" step="0.01" value="129" required />
            </div>
          </div>
          <div class="field"><label>Description</label><input name="desc" required /></div>
          <div class="field"><label>Product Image (Optional)</label>
          ...
          ?>
            <div class="chip" style="display:flex;align-items:center;gap:0.4rem;padding-right:0.4rem;">
              ...
              <form method="POST" style="margin:0;" onsubmit="return confirm('Delete the category <?= htmlspecialchars($c['name']) ?>?');">
                ...
                <button type="submit" name="delete_category" class="btn ghost sm"
                        style="padding:0.1rem 0.4rem;border:none;font-size:0.75rem;min-width:auto;">✕</button>
              </form>
            </div>
          <?php endwhile; ?>
        </div>
            ...
            <tr>
              ...
              <td class="right">
                <form method="POST" style="margin:0"
                      onsubmit="return confirm('Are you sure you want to delete <?= htmlspecialchars($p['name']) ?>?');">
                  <?= csrf_field() ?>
